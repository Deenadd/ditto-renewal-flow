...
lib/
  ...
  quote-context.tsx    V2's price, carried through the rest of the journey
...
A changed cover carries its price with it: the sidebar card and policy bar on
...
The cover pickers, add-on rows and policy-period cards hide their radios and
checkboxes to draw their own, so each wrapping label takes a focus ring while
the input inside it has keyboard focus. Text inputs render at 16px below `sm`,
which stops iOS Safari zooming the page whenever a field is focused; desktop
sizes are unchanged.
